...
// Get settings
$query_settings = $database->query("SELECT * FROM `".TABLE_PREFIX."mod_jqCollapse_settings` WHERE section_id='$section_id' LIMIT 1");
$fetch_settings = $query_settings->fetchRow();

$header= $admin->strip_slashes($fetch_settings['header']);
$footer= $admin->strip_slashes($fetch_settings['footer']);
$color= $admin->strip_slashes($fetch_settings['color']);
$gradient= $admin->strip_slashes($fetch_settings['gradient']);
$icon= $admin->strip_slashes($fetch_settings['icon']);
$fontcolor= $admin->strip_slashes($fetch_settings['fontcolor']);
...
if($query_cats->numRows() > 0)
{
	while($cat = $query_cats->fetchRow())
	{
		...
		$query_quests = $database->query("SELECT * FROM `".TABLE_PREFIX."mod_jqCollapse_questions` WHERE cat_id='".$cat['cat_id']."' and section_id='".$section_id."' ORDER BY pos ASC");
...
if($query_cats->numRows() > 0)
{
	while($cat = $query_cats->fetchRow())
	{
		...
		if($query_quests->numRows() > 0)
		{
			echo '<div class="accordion cat_id-'.$cat['cat_id'].' fontcolor_'.$fontcolor.' '.$color.' '.$gradient.' '.$icon.'">';
			while($quest = $query_quests->fetchRow())
			{
				...
				$content = '<div class="collapse-contentblock">'.$content.'</div>';
